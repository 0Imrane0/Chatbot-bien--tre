...
class ResponseGenerator:
    ...
    def __init__(self, enable_cbt: bool = True):
        ...
        Args:
            enable_cbt: Activer le module CBT (par défaut: True)
        """
        print("🔧 Initialisation du Générateur de Réponses...")
        
        # ============================================
        # MODULE CBT (Thérapie Cognitivo-Comportementale)
        # ============================================
        self.enable_cbt = enable_cbt
        if self.enable_cbt:
            try:
                self.cbt_engine = CBTEngine()
                print("   ✅ Module CBT activé")
            except Exception as e:
                print(f"   ⚠️ Erreur CBT: {e}")
    ...
    def generate_response(self, sentiment: str, sentiment_detail: str,
        ...
        if self.enable_cbt and sentiment in ['négatif', 'très négatif', 'negative', 'negatif', 'tres negatif']:
            try:
                ...
                cbt_data = self.cbt_engine.generate_cbt_response(
                    user_message=text,
                    sentiment=sentiment,
                    emotional_intensity=1.0 - confidence  # Inverser pour intensité
                )
                
                # Si c'est une crise détectée par CBT, priorité absolue
                if cbt_data.get('is_crisis', False):
                    return {
                        'main_response': cbt_data['restructuring'],
                        'advice': [],
                        'encouragement': '',
                        'is_crisis': True,
        ...
        # ========================================
        # ÉTAPE FINALE : ENRICHIR AVEC CBT
        # ========================================
        if cbt_response and self.enable_cbt:
            # Ajouter les informations CBT à la réponse
            cbt_formatted = self.cbt_engine.format_response_for_user(cbt_response)
            
            # Enrichir la réponse principale avec CBT
            if cbt_response.get('restructuring') or cbt_response.get('questions'):
                main_response = main_response + trend_comment + "\n\n" + cbt_formatted
            
            # Ajouter les actions CBT aux conseils
            if cbt_response.get('actions'):
                for action in cbt_response['actions'].get('immediate', []):
                    if action not in advice_list:
